...
Revisa `template.yaml` y `samconfig.toml.example`, copia la configuración a un archivo local no versionado y ejecuta siempre `sam deploy --guided` (o `sam deploy` con una configuración ya revisada). SAM debe subir los artefactos a S3 y resolverlos en CloudFormation: la capa de navegador real mide aproximadamente 71,2 MiB comprimida (74 670 009 bytes), por encima del límite de 50 MiB de carga ZIP directa de Lambda. No uses `aws lambda publish-layer-version --zip-file`, `aws lambda create-function --zip-file` ni otro flujo de carga directa. Conserva los nombres predeterminados de los parámetros SSM salvo que exista una convención aprobada. El despliegue no activa el monitor: no ejecutes el script de activación durante la validación inicial. En cuentas nuevas, deja `EnableReservedConcurrency=false` (valor predeterminado), porque AWS exige mantener al menos 10 ejecuciones Lambda sin reservar; la cola, el bloqueo condicional y el lote de un mensaje conservan la ejecución serializada. Actívalo únicamente si la cuota de concurrencia permite reservar una ejecución para cada función.
