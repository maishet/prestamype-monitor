...
const text =
  ...
  "✅ Prestamype monitor: mensaje de prueba Telegram";
  ...
  if (!response.ok || payload.ok !== true) {
    const reason =
      typeof payload.description === "string"
        ...
        : ".";
    throw new Error(
      `Telegram rechazó el destino ${destination} (HTTP ${response.status})${reason}`,
    );
  }
  console.log(
    `Telegram respondió correctamente para el destino ${destination}.`,
  );
}
